...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		var foldersToCheck:Array<String> = [Paths.getPreloadPath('data/scripts/')];

		#if MODS_ALLOWED
		foldersToCheck.insert(0, Paths.mods('data/scripts/'));
		if(Paths.currentModDirectory != null && Paths.currentModDirectory.length > 0)
			foldersToCheck.insert(0, Paths.mods(Paths.currentModDirectory + '/data/scripts/'));

		for(mod in Paths.getGlobalMods())
			foldersToCheck.insert(0, Paths.mods(mod + '/data/scripts/'));
		#end
		...
		{
			#if MODS_ALLOWED
			var luaToLoad:String = Paths.modFolders('data/custom_notetypes/' + notetype + '.lua');
			...
			{
				luaToLoad = Paths.getPreloadPath('data/custom_notetypes/' + notetype + '.lua');
				if(FileSystem.exists(luaToLoad))
				{
					luaArray.push(new FunkinLua(luaToLoad));
				}
			...
			#elseif sys
			var luaToLoad:String = Paths.getPreloadPath('data/custom_notetypes/' + notetype + '.lua');
		...
		{
			#if MODS_ALLOWED
			var luaToLoad:String = Paths.modFolders('data/custom_events/' + event + '.lua');
			if(FileSystem.exists(luaToLoad))
			{
				luaArray.push(new FunkinLua(luaToLoad));
			}
			...
			{
				luaToLoad = Paths.getPreloadPath('data/custom_events/' + event + '.lua');
				if(FileSystem.exists(luaToLoad))
				{
					luaArray.push(new FunkinLua(luaToLoad));
				}
			...
			#elseif sys
			var luaToLoad:String = Paths.getPreloadPath('data/custom_events/' + event + '.lua');
			if(OpenFlAssets.exists(luaToLoad))
			{
				luaArray.push(new FunkinLua(luaToLoad));
			}
	...
	public function initLuaShader(name:String, ?glslVersion:Int = 120)
	{
		...
		if(Paths.currentModDirectory != null && Paths.currentModDirectory.length > 0)
			foldersToCheck.insert(0, Paths.mods(Paths.currentModDirectory + '/data/shaders/'));
	...
	private function generateSong(dataPath:String):Void
	{
		...
		var songName:String = Paths.formatToSongPath(SONG.song);
		var file:String = Paths.json('chart/' + songName + '/events');
		#if MODS_ALLOWED
		if (FileSystem.exists(Paths.modsJson('chart/' + songName + '/events')) || FileSystem.exists(file)) {
		#else
		if (OpenFlAssets.exists(file)) {
		#end
