	...
	**/
	...
	function addSongUI():Void
	{
		...
		var loadEventJson:FlxButton = new FlxButton(loadAutosaveBtn.x, loadAutosaveBtn.y + 30, 'Load Events', function()
		{
			...
			#if sys
			if (#if MODS_ALLOWED FileSystem.exists(Paths.modsJson('chart/' + songName + '/events')) || #end FileSystem.exists(file))
			#else
		...
		#if MODS_ALLOWED
		var directories:Array<String> = [Paths.mods('data/characters/'), Paths.mods(Paths.currentModDirectory + '/data/characters/'), Paths.getPreloadPath('data/characters/')];
		...
		#if MODS_ALLOWED
		var directories:Array<String> = [Paths.mods('data/stages/'), Paths.mods(Paths.currentModDirectory + '/data/stages/'), Paths.getPreloadPath('data/stages/')];
		for(mod in Paths.getGlobalMods())
		...
		for(i in 0...CoolUtil.difficulties.length){
			...
			var pathExists:Bool = Paths.fileExists('data/chart/' + formattedFolder + '/' + formattedSong + '.json', BINARY);
			if(pathExists == true){
	...
	function addNoteUI():Void
	{
		...
		#if MODS_ALLOWED
		directories.push(Paths.mods('data/custom_notetypes/'));
		directories.push(Paths.mods(Paths.currentModDirectory + '/data/custom_notetypes/'));
		for(mod in Paths.getGlobalMods())
	...
	function addEventsUI():Void
	{
		...
		#if MODS_ALLOWED
		directories.push(Paths.mods('data/custom_events/'));
		directories.push(Paths.mods(Paths.currentModDirectory + '/data/custom_events/'));
		for(mod in Paths.getGlobalMods())
			directories.push(Paths.mods(mod + '/data/custom_events/'));
		#end
